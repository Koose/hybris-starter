	...
	<c:choose>
		<c:when test="${cartPage}">
			...
			<a href="javascript:void(0)"
			   class="btn btn-secondary btn-block js-pickup-in-store-button"
		...
		<c:when test="${searchResultsPage}">
			<button class="btn btn-secondary btn-block js-pickup-in-store-button icon-location" disabled="disabled" id="product_${fn:escapeXml(product.code)}${entryNumber}" type="button submit"
		...
		<c:otherwise>
			<button class="btn btn-secondary btn-block js-pickup-in-store-button icon-location btn-icon" disabled="disabled" id="product_${fn:escapeXml(product.code)}${entryNumber}" type="submit" data-productavailable="${product.availableForPickup}"
			data-productcart='${fn:escapeXml(product.price.formattedValue)}'
			data-productcart-variants='{
					<c:if test="${not empty product.baseOptions[0].selected.variantOptionQualifiers}">
						<c:forEach var="variant" items="${product.baseOptions[0].selected.variantOptionQualifiers}" varStatus="productcartVariantsNumber">

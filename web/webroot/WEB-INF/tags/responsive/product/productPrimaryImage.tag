	...
	<c:when test="${not empty primaryImage}">
		...
		<c:choose>
			<c:when test="${not empty primaryImage.altText}">
				...
				<img class="${cssClass} lazy" data-src="${primaryImageUrl}" alt="${altText}" title="${altText}"/>
			</c:when>
			<c:otherwise>
                <img class="${cssClass} lazy" data-src="${primaryImageUrl}" alt="${productName}" title="${productName}"/>
			</c:otherwise>
		</c:choose>
	</c:when>
	<c:otherwise>
		<theme:image code="img.missingProductImage.responsive.${format}" alt="${productName}" title="${productName}" cssClass="${cssClass}"/>
	</c:otherwise>
</c:choose>

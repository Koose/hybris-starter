			...
			<div class="text-${textPos}">
				...
				<c:choose>
					<c:when test="${not showModifications}">
						<div class="row">
							<div class="col-6 col-sm-4 col-md-3 col-lg-2 col-offset-3 col-sm-offset-4 col-md-offset-4 col-lg-offset-5">
								<br />
								<button class="btn btn-secondary btn-block cartRestoration" data-cart-url="${cartUrl}"><spring:theme text="View Cart" code="basket.restoration.view.cart.btn"/></button>
							</div>
						</div>
					</c:when>
					<c:otherwise>

...
<c:if test="${not empty order.b2bPermissionResult}">
	<div class="account-orderdetail card bg-light">
        <div class="card-header">
            <spring:theme code="text.account.order.approval.title" />
        </div>
    </div>
    <div class="order-approval-list card-body">
        <table class="table orderListTable responsive-table">
        <thead>
        ...
        <tbody>
            <c:forEach items="${order.b2bPermissionResult}" var="b2bPermissionResult">
                <tr class="responsive-table-item">
                    <td class="d-md-none"><spring:theme code="text.account.order.permission.title" /></td>
                    <td class="responsive-table-cell">${fn:escapeXml(b2bPermissionResult.permissionTypeData.name)}</td>

                    <td class="d-md-none"><spring:theme code="text.account.order.approver.title" /></td>
                    <td class="responsive-table-cell">${fn:escapeXml(b2bPermissionResult.approver)}</td>

                    <td class="d-md-none"><spring:theme code="text.account.order.status.title" /></td>
                    <td class="responsive-table-cell">${fn:escapeXml(b2bPermissionResult.statusDisplay)}</td>

                    <td class="d-md-none"><spring:theme code="text.account.order.permission.notes"/></td>

...
<div class="c-add-to-cart">
	...
	<form:form method="post" id="configureForm" class="c-add-to-cart__configure-form configure_form" action="${configureProductUrl}">
	...
	<form:form method="post" id="addToCartForm" class="c-add-to-cart__form add_to_cart_form" action="${addToCartUrl}">
		<c:if test="${product.purchasable}">
			<input type="hidden" maxlength="3" size="1" id="qty" name="qty" class="qty js-qty-selector-input" value="1">
		</c:if>
		<input type="hidden" name="productCodePost" value="${fn:escapeXml(product.code)}"/>
		...
		<c:if test="${empty showAddToCart ? true : showAddToCart}">
			...
			<c:choose>
				<c:when test="${fn:contains(buttonType, 'button')}">
					<button type="${buttonType}" class="btn btn-primary btn-block js-add-to-cart btn-icon outOfStock" disabled="disabled">
				...
				<c:otherwise>
					<ycommerce:testId code="addToCartButton">
						<button id="addToCartButton" type="${buttonType}" class="btn btn-primary btn-block c-add-to-cart__button js-enable-btn btn-icon" disabled="disabled">
							<span class="icon-cart"></span>
